...
bool VideoCapturer::StartCapturing(const VideoFormat& capture_format) {
  LOG(LS_INFO) << "FUNC:" << __FUNCTION__ << "  Camera '" << GetId() << "' started with format " << capture_format.ToString();
...
//跟新Sink接口
void VideoCapturer::AddOrUpdateSink(rtc::VideoSinkInterface<cricket::VideoFrame>* sink, const rtc::VideoSinkWants& wants) 
{
	RTC_DCHECK(thread_checker_.CalledOnValidThread());
...
//将数据发送给 videobroadcaster 言梧栩
void VideoCapturer::OnFrameCaptured(VideoCapturer*,  const CapturedFrame* captured_frame) {
  int out_width;
  int out_height;
  int crop_width;
  int crop_height;
  ...
  if (!AdaptFrame(captured_frame->width, captured_frame->height,captured_frame->time_stamp,
  ...
  if (!adapted_frame) {
    ...
    LOG(LS_ERROR) << "Couldn't convert to I420! " << "From " << ToString(captured_frame) << " To "
...
void VideoCapturer::OnFrame(const VideoFrame& frame, int orig_width, int orig_height) 
{
...
int64_t VideoCapturer::GetFormatDistance(const VideoFormat& desired, const VideoFormat& supported) {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
