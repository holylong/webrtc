...
//根性Sink队列  言梧栩
void VideoBroadcaster::AddOrUpdateSink(VideoSinkInterface<cricket::VideoFrame>* sink, const VideoSinkWants& wants) 
{
...
//从VideoCapture中获取视频流 将流发送到WebRtcVideoSendStream 类型，此类输入WebRtcVideoChannel2内部类  言梧栩
void VideoBroadcaster::OnFrame(const cricket::VideoFrame& frame) {
  ...
  for (auto& sink_pair : sink_pairs())
  {
	...
	{
      sink_pair.sink->OnFrame(cricket::WebRtcVideoFrame(GetBlackFrameBuffer(frame.width(), frame.height()),
