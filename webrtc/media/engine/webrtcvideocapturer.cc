...
class WebRtcVcmFactory : public WebRtcVcmFactoryInterface {
 public:
  ...
  {
	  LOG(LS_INFO) << "=======>>>>VideoCaptureFactory in WebRtcVcmFactory Create VideoCaptureModule";
...
///////////////////////////////////////////////////////////////////////////
//WebRtcVideoCapturer 构造函数 创建 WebRtcVcmFactory 言梧栩
...
bool WebRtcVideoCapturer::Init(const Device& device) 
{
  ...
  for (int index = 0; index < num_cams; ++index) {
    char vcm_name[256];
    if (info->GetDeviceName(index, vcm_name, arraysize(vcm_name), vcm_id, arraysize(vcm_id)) != -1) {
...
bool WebRtcVideoCapturer::Init(const rtc::scoped_refptr<webrtc::VideoCaptureModule>& module) {
  RTC_DCHECK(!start_thread_);
  if (module_) {
    LOG(LS_ERROR) << "The capturer is already initialized";
    return false;
...
CaptureState WebRtcVideoCapturer::Start(const VideoFormat& capture_format) {
  ...
  LOG(LS_INFO) << "Camera '" << GetId() << "' started with format " << capture_format.ToString() << ", elapsed time "<< rtc::TimeSince(start) << " ms";
...
//从VideoCaptureImpl 对象中获取的数据 言梧栩
void WebRtcVideoCapturer::OnIncomingCapturedFrame(const int32_t id, const webrtc::VideoFrame& sample) {
  ...
  else
  {
		...
		// Note that Bind captures by value, so there's an intermediate copy of sample.
		async_invoker_->AsyncInvoke<void>(RTC_FROM_HERE, start_thread_,rtc::Bind(&WebRtcVideoCapturer::SignalFrameCapturedOnStartThread, this, sample));
  }
}
...
void WebRtcVideoCapturer::SignalFrameCapturedOnStartThread(const webrtc::VideoFrame& frame)
{
	// This can only happen between Start() and Stop().
	RTC_DCHECK(start_thread_);
	RTC_DCHECK(start_thread_->IsCurrent());
