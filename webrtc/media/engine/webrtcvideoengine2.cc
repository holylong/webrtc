...
std::string WebRtcVideoChannel2::CodecSettingsVectorToString( const std::vector<VideoCodecSettings>& codecs)
{
  std::stringstream out;
  out << '{';
  for (size_t i = 0; i < codecs.size(); ++i) {
...
bool WebRtcVideoChannel2::SetVideoSend(uint32_t ssrc,bool enable,
	const VideoOptions* options, rtc::VideoSourceInterface<cricket::VideoFrame>* source) 
{
...
//初始化WebRtcVideoSendStream 对象 并将Call*成员变量初始化 言梧栩
...
//将视频流插入到VideoCaptureInput中 言梧栩
void WebRtcVideoChannel2::WebRtcVideoSendStream::OnFrame( const VideoFrame& frame) 
{
  TRACE_EVENT0("webrtc", "WebRtcVideoSendStream::OnFrame");
  webrtc::VideoFrame video_frame(frame.video_frame_buffer(), 0, 0, frame.rotation());
...
void WebRtcVideoChannel2::WebRtcVideoSendStream::FillBandwidthEstimationInfo(BandwidthEstimationInfo* bwe_info) 
{
  rtc::CritScope cs(&lock_);
  if (stream_ == NULL) {
    return;
...
//RecreateWebRtcStream  创建VideoSendStream 如果已初始化就直接Destory重新创建  言梧栩
void WebRtcVideoChannel2::WebRtcVideoSendStream::RecreateWebRtcStream() {
  ...
  //在WebRtcVideoSendStream中创建 VideoSendSream对象 言梧栩
...
//初始化WebRtcVideoReceiveStream 对象 并将Call*成员变量初始化 言梧栩
