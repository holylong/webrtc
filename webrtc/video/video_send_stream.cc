...
//初始化VideoSendStream 将VideoSendInput插到成员变量中，并实时将里边的数据取出 言梧栩
...
{
  RTC_CHECK_EQ(0, vie_encoder_.RegisterExternalEncoder( config_.encoder_settings.encoder,
                      config_.encoder_settings.payload_type, config_.encoder_settings.internal_source));
  ...
  while (true) {
    ...
    if (change_settings) {
      current_encoder_settings_->video_codec.startBitrate = std::max(bitrate_allocator_->GetStartBitrate(this) / 1000,
