...
rtc::scoped_refptr<VideoCaptureModule> VideoCaptureImpl::Create( const int32_t id,  VideoCaptureExternal*& externalCapture) 
{
  LOG(LS_INFO) << "=======>>>>VideoCaptureImpl new rtc::RefCountedObject<VideoCaptureImpl>";
  rtc::scoped_refptr<VideoCaptureImpl> implementation( new rtc::RefCountedObject<VideoCaptureImpl>(id));
...
//注册callback 用于获取视频数据 言梧栩
void VideoCaptureImpl::RegisterCaptureDataCallback(VideoCaptureDataCallback& dataCallBack)
{
...
int32_t VideoCaptureImpl::IncomingFrame( uint8_t* videoFrame, size_t videoFrameLength,
	const VideoCaptureCapability& frameInfo, int64_t captureTime/*=0*/)
{
    ...
    if (frameInfo.codecType == kVideoCodecUnknown)
    {
        ...
        const VideoType commonVideoType = RawVideoTypeToCommonVideoVideoType(frameInfo.rawType);
        ...
        const int conversionResult = ConvertToI420(commonVideoType, videoFrame, 0, 0, width, height, videoFrameLength, apply_rotation ? _rotateFrame : kVideoRotation_0, &_captureFrame);

		//LOG(LS_INFO) << "==>>res:" << conversionResult << " videotype:" << commonVideoType << " width:" << width << " height:" << height << " frame len:" << videoFrameLength << " apply_rotation:" << apply_rotation
			//<< " _captureFrame:" << _captureFrame.height();
