...
bool ScreenCapturerLinux::Init(const DesktopCaptureOptions& options) {
  ...
  if (root_window_ == BadValue) {
    LOG(LOG) << "Unable to get the root window";
    DeinitXlib();
    return false;
  }
  ...
  if (gc_ == NULL) {
    LOG(LOG) << "Unable to get graphics context";
    DeinitXlib();
    return false;
  }
  ...
  if (!x_server_pixel_buffer_.Init(display(), DefaultRootWindow(display()))) {
    LOG(LOG) << "Failed to initialize pixel buffer.";
    return false;
  }
...
void ScreenCapturerLinux::InitXDamage() {
  ...
  if (!damage_handle_) {
    LOG(LOG) << "Unable to initialize XDamage.";
    return;
  }
  ...
  if (!damage_region_) {
    ...
    LOG(LOG) << "Unable to create XFixes region.";
    return;
  }
...
void ScreenCapturerLinux::ScreenConfigurationChanged() {
  ...
  if (!x_server_pixel_buffer_.Init(display(), DefaultRootWindow(display()))) {
    LOG(LOG) << "Failed to initialize pixel buffer after screen "
