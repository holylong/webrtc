...
void WindowCapturerWin::Capture(const DesktopRegion& region) {
   //std::cout << "mvp Capture" << std::endl;
  if (!window_) {
    LOG(LS_WARNING) << "Window hasn't been selected: " << GetLastError();
    callback_->OnCaptureResult(Result::ERROR_PERMANENT, nullptr);
	//LOG(LS_INFO) << "mvp ERROR_PERMANENT";
	std::cout << "mvp ERROR_PERMANENT" << std::endl;
    return;
  }
  ...
  if (!IsWindow(window_)) {
    callback_->OnCaptureResult(Result::ERROR_PERMANENT, nullptr);
	std::cout << "ERROR_PERMANENT:" << (int)(Result::ERROR_PERMANENT) << std::endl;
    return;
  }
  ...
  if (IsIconic(window_) || !IsWindowVisible(window_)) {
	...
	std::cout << "SUCCESS:" << (int)(Result::SUCCESS) << std::endl;
    return;
  }
  ...
  if (!GetCroppedWindowRect(window_, &cropped_rect, &original_rect)) {
	...
	std::cout << "ERROR_TEMPORARY:" << (int)(Result::ERROR_TEMPORARY) << std::endl;
    return;
  }
  ...
  if (!result) {
    LOG(LS_WARNING) << "Both PrintWindow() and BitBlt() failed.";
    frame.reset();
  }
