 ...
 public:
  ...
  bool CopyFrom(const DXGI_OUTDUPL_FRAME_INFO& frame_info,
    ...
    if (error.Error() != S_OK || !texture) {
      LOG(LS_WARNING) << "Failed to convert IDXGIResource to ID3D11Texture2D, "
                       "error "
    ...
    if (error.Error() != S_OK) {
      rect_ = {0};
      LOG(LS_WARNING) << "Failed to map the IDXGISurface to a bitmap, error "
                    << error.ErrorMessage() << ", code " << error.Error();
      return false;
    }
  ...
  bool InitializeStage(ID3D11Texture2D* texture) {
    ...
    if (stage_) {
      ...
      if (error.Error() == S_OK) {
        ...
        LOG(LS_WARNING) << "Failed to unmap surface, error "
                      << error.ErrorMessage() << ", code " << error.Error();
      }
    ...
    if (error.Error() != S_OK || !stage_) {
      LOG(LS_WARNING) << "Failed to create a new ID3D11Texture2D as stage, "
                       "error "
                    << error.ErrorMessage() << ", code " << error.Error();
      return false;
    }
    ...
    if (error.Error() != S_OK || !surface_) {
      LOG(LS_WARNING) << "Failed to convert ID3D11Texture2D to IDXGISurface, "
                       "error "
                    << error.ErrorMessage() << ", code " << error.Error();
      return false;
    }
...
bool ScreenCapturerWinDirectx::DuplicateOutput() {
  ...
  if (g_container->duplication_desc.ModeDesc.Format !=
    ...
    g_container->duplication.Reset();
    LOG(LS_WARNING) << "IDXGIDuplicateOutput does not use RGBA (8 bit) "
...
bool ScreenCapturerWinDirectx::HandleDetectUpdatedRegionError(
  ...
  if (error.Error() != S_OK) {
    if (error.Error() == DXGI_ERROR_ACCESS_LOST) {
      ...
      LOG(LS_WARNING) << "Failed to get " << stage << " rectangles, error "
                    << error.ErrorMessage() << ", code " << error.Error();
    }
...
bool ScreenCapturerWinDirectx::DetectUpdatedRegion(
  ...
  if (frame_info.TotalMetadataBufferSize == 0) {
    ...
    LOG(LS_WARNING) << "frame_info.AccumulatedFrames > 0, "
...
std::unique_ptr<DesktopFrame> ScreenCapturerWinDirectx::ProcessFrame(
  ...
  if (shared_memory_factory_) {
    ...
    if (!frames_.current_frame() ||
      ...
      if (!new_frame) {
        LOG(LS_WARNING) << "Failed to allocate a new SharedMemoryDesktopFrame";
        return std::unique_ptr<DesktopFrame>();
      }
...
void ScreenCapturerWinDirectx::Capture(const DesktopRegion& region) {
  ...
  if (error.Error() != S_OK) {
    LOG(LS_WARNING) << "Failed to capture frame, error " << error.ErrorMessage()
                  << ", code " << error.Error();
