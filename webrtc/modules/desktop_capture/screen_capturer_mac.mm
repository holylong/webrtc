...
bool ScreenCapturerMac::CgBlitPostLion(const DesktopFrame& frame,
  ...
  if (current_display_) {
    ...
    if (config) {
      ...
      LOG(LOG) << "The selected screen cannot be found for capturing.";
      return false;
    }
  } else {
  ...
  for (size_t i = 0; i < displays_to_capture.size(); ++i) {
    ...
    if (bits_per_pixel / 8 != DesktopFrame::kBytesPerPixel) {
      LOG(LOG) << "CGDisplayCreateImage() returned imaged with "
                    << bits_per_pixel
...
void ScreenCapturerMac::ScreenConfigurationChanged() {
  ...
  if (!app_services_library_) {
    LOG_F(LOG) << "Failed to open " << kApplicationServicesLibraryName;
    abort();
  }

  opengl_library_ = dlopen(kOpenGlLibraryName, RTLD_LAZY);
  if (!opengl_library_) {
    LOG_F(LOG) << "Failed to open " << kOpenGlLibraryName;
    abort();
  }
...
bool ScreenCapturerMac::RegisterRefreshAndMoveHandlers() {
  ...
  if (err != kCGErrorSuccess) {
    LOG(LOG) << "CGRegisterScreenRefreshCallback " << err;
    return false;
  }
  ...
  if (err != kCGErrorSuccess) {
    LOG(LOG) << "CGScreenRegisterMoveCallback " << err;
    return false;
  }
