...
//创建应答信息			言梧栩
void WebRtcSession::CreateAnswer(CreateSessionDescriptionObserver* observer,  const cricket::MediaSessionOptions& session_options) 
{
	webrtc_session_desc_factory_->CreateAnswer(observer, session_options);
}

bool WebRtcSession::SetLocalDescription(SessionDescriptionInterface* desc,  std::string* err_desc)
{
  ASSERT(signaling_thread()->IsCurrent());
  ...
  cricket::SecurePolicy sdes_policy = webrtc_session_desc_factory_->SdesPolicy();
  cricket::CryptoType crypto_required = dtls_enabled_ ? cricket::CT_DTLS : (sdes_policy == cricket::SEC_REQUIRED ?
...
bool WebRtcSession::SetRemoteDescription(SessionDescriptionInterface* desc,  std::string* err_desc)
{
  ASSERT(signaling_thread()->IsCurrent());
