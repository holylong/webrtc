...
rtc::scoped_refptr<VideoTrackSourceInterface>   PeerConnectionFactory::CreateVideoSource( 
	cricket::VideoCapturer* capturer, const MediaConstraintsInterface* constraints) 
{
	LOG(INFO) << "FUNCNAME:" << __FUNCTION__ << " INFO:" << "Create Video Track Source";
  RTC_DCHECK(signaling_thread_->IsCurrent());
  rtc::scoped_refptr<VideoTrackSourceInterface> source( VideoCapturerTrackSource::Create(worker_thread_, capturer, constraints,false));
  return VideoTrackSourceProxy::Create(signaling_thread_, worker_thread_, source);
}
...
{
	LOG(INFO) << "FUNCNAME:" << __FUNCTION__ << " INFO:" << "Create Video Track";
  RTC_DCHECK(signaling_thread_->IsCurrent());
