...
bool Conductor::CreatePeerConnection(bool dtls) {
  ...
  if (dtls) {
	...
	//constraints.AddOptional(webrtc::MediaConstraintsInterface::kScreencastMinBitrate, "true");
...
cricket::VideoCapturer* Conductor::OpenVideoCaptureDevice() {
  ...
  cricket::FakeScreenCapturerFactory screenfactory;
  cricket::VideoCapturer* capturer = nullptr;
  for (const auto& name : device_names) {
    capturer = factory.Create(cricket::Device(name, 0));
	//rtc::DesktopId did;
	//rtc::WindowId wid = (rtc::WindowId)GetDesktopWindow();
	//cricket::ScreencastId* scid = new cricket::ScreencastId(wid);
	//capturer = screenfactory.Create(*scid);
    if (capturer) {
      break;
    }
  }
